...
è comunque possibile che modifichino il codice nello stesso momento.
Se ognuno modifica una copia diversa, queste vanno poi unite sempre a mano con
dei copia e incolla.

Un sistema di controllo di versione tiene traccia di tutte le modifiche
effettuate ad un insieme di file (che compongono il progetto), di chi le
ha fatte, quando, quali righe sono state tolte e quali aggiunte.
Ogni registrazione di modifica è associata ad un titolo e ad una descrizione che
la spiega in dettaglio, in modo da facilitarne la comprensione ad altri sviluppatori
e la rilettura in futuro
è possibile tornare a qualsiasi punto della storia del progetto.
...
esempio), e dopo aver apportato le modifiche necessarie è possibile caricarle
sul server e unirle a quelle degli altri tenendo traccia di chi ha modificato
...
opensource. è scritto principalmente in C e shell.
...
\code{git}. Ognuno può accettare un diverso numero di argomenti e di opzioni
...
molto lunga e dettagliata per spiegare in modo discorsivo cosa si è fatto e se
eventualmente ha causato dei problemi.
...
\link{https://chris.beams.io/posts/git-commit/}{Questo} è un buon
...
\subsection{Add, reset, status}
I file coinvolti dal commit devono essere prima selezionati con add. In questo
modo si entra nella \textbf{staging area}. è uno stato intermedio che sta prima
di un commit per tracciare le modifiche momentanee in caso di più prove.
In un determinato momento, si può decidere di mettere le modifiche effettuate ad
alcuni file nella staging area per poi committarle, mentre quelle effettate su
altri lasciarle nello stadio precedente, chiamato \textbf{working tree},
anche per diversi commit di fila; non verranno incluse in alcun commit fino a
quando non le si aggiugerà alla staging area.
...
Il comando \code{git reset} fa il contrario di add, rimuovendo dalla staging
area i file o cartelle passati per argomento; invocato senza argomento li
rimuove tutti. Può essere molto utile se si sono svolte contemporaneamente
modifiche che si vuole dividere in commit diversi.
...
Fetch da solo tuttavia non modifica mai alcun file sul repository locale.
...
remota del branch su cui si è localmente, ma nel caso di merge è aggiornata
all' ultimo fetch.
...
La lunga serie di caratteri sono i codici \textbf{hash}, univoci per ogni
...
argomento di log subito dopo \code{--grep=} (grep è un altro tool unix
utilizzato da git):
...
precedute da un \code{<} se quella determinata riga è presente solo nel file
passato come primo argomento, altrimenti \code{>}.
...
Dietro a questi tre nomi ci sono concetti importanti di git che tornano utili
con la maggior parte dei comandi
...
Il numero dopo il carattere \code{\textasciitilde} indica di quanti commit si
deve tornare indietro per trovare il commit su cui effettaure il checkout.
è un eccezzione il fatto che \code{\textasciitilde} e \code{\textasciitilde1}
siano equivalenti.
...
o delle modifiche non ancora committate.
...
modifiche che riportano allo stato del commit di cui si è specificato il codice
come argomento di \textbf{revert}.

\code{..HEAD} indica che si ripristinano le modifiche
...
su sulle le novità portate da quella release. Come per i commit, l'opzione -m
...
L'output è molto simile a quello di \code{git log}. Se si vuole vedere le
descrizioni di tutti i tag basta chiamare \code{git show} senza argomenti.
...
\subsection{Remotes\label{remoti}}
...
distribuzione ed è un processo che richiede pochi secondi.
...
Per impostare editor e pager utilizzati da git (di solito vim e less sono già 
...
Scrivere \code{git} davanti ad ogni comando può risultare scomodo.
Esiste un modo per evitarlo, sfruttando le funzionalità di alias messe
a disposizione dalla shell. Per impostarli occorre modificare il file di
configurazione della propria shell, \code{\textasciitilde/.bashrc} 
se si utilizza bash.
...
verranno richiesti al push su un qualsiasi repository Github e anche
...
Per aggiornare i submodule occorre andare nella loro cartella e aggiornarli alle
modifiche remote come se il proprio repository (quindi con pull ecc). è bene
eseguire un commit in cui si effettua solamente l'aggiornamento dei submodule
per non confonderlo con le effettive modifiche del proprio repository.

Da notare come viene aggiornato il file \code{.gitmodules}, da cui è possibile
...
riposizionarli nello stesso punto infatti lo stesso comando senza \code{-i} non
...
andrebbero persi per sempre. \code{push -f} forza la loro cancellazione.
...
\begin{itemize}
    \item \link{https://stackoverflow.com/questions/179123/how-to-modify-existing-unpushed-commit-messages}
    ...
    \item \link{https://www.atlassian.com/git/tutorials/rewriting-history/git-rebase}
        {git rebase}
    \item \link{https://www.atlassian.com/git/tutorials/merging-vs-rebasing}
        {merge vs rebase}
    ...
    \item \link{https://stackoverflow.com/questions/5828324/update-git-submodule-to-latest-commit-on-origin}
